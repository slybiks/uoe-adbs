...
import ed.inf.adbs.minibase.evaluation.DatabaseCatalog;
...
public class Minibase {
    ...
    public static void evaluateCQ(String databaseDir, String inputFile, String outputFile) {
        ...
        try {
            ...
            catalog.initialise(databaseDir);

            Query inputQuery = QueryParser.parse(Paths.get(inputFile));

            QueryPlanner queryPlanner = QueryPlanner.getQueryPlanner();
            queryPlanner.initialiseQueryPlanner(inputQuery);

            Utils.createFileWriter(outputFile);
            queryPlanner.getRootOperator().dump();
        } catch (Exception e) {
            System.err.println("An error occurred while evaluating the conjunctive query");
            e.printStackTrace();
        }
    }
